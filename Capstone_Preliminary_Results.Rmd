...
# aaron's read.csv -- commented out bc I have a different file path, just comment mine out when you rerun!
...
for (i in 1:10) 
  {
  glm.fit <- glm(formula=Class ~ energy + loudness + mode + acousticness + valence + duration_in.min.ms, family=binomial, data=mi)
  error_df[i, 'test_error'] <- cv.glm(mi, glm.fit , K = 10)$delta[1]
  error_df[i, 'fold'] <- i
}
...
ggplot(error_df,
  ...
  geom_line() + labs(x = 'Fold', y = 'Test Error', title = "")
```
