...
genre <- na.omit(genre)
View(genre)
mi <- subset(genre, Class == c(5,9))
mi$Class <- ifelse(mi$Class==5, 0, 1)
View(mi)
...
updated_model <- glm(formula=Class ~ Popularity + danceability + speechiness + valence, family=binomial, data=mi, subset=train)
updated.probs = predict(updated_model, mi_test, type="response")

updated.pred=rep(0, 417)
