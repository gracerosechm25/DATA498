...
# Alt (1) vs Indie Alt (6)
genre <- read.csv("~/DATA 498A HW/Capstone Project/train.csv")
genre <- na.omit(genre)
View(genre)
mi <- subset(genre, Class == c(1,6))
mi$Class <- ifelse(mi$Class==1, 0, 1)
View(mi)
...
updated_model <- glm(formula=Class ~ Popularity + instrumentalness, family=binomial, data=mi, subset=train)
updated.probs = predict(updated_model, mi_test, type="response")

updated.pred=rep(0, 737)
